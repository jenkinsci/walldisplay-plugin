...
public class WallDisplayPlugin extends Plugin {
    ...
    private static final Logger LOGGER = Logger.getLogger("hudson."
	    + WallDisplayPlugin.class.getName());

    public final static String PLUGIN_NAME = "jenkinswalldisplay";

    public static final String[] themes = new String[] { "Default", "Plain",
	    "Christmas", "Boss", "Dark", "Colorblind", "Custom" };

    public static final String[] fontFamilies = new String[] { "Sans-Serif",
	    "Arial", "Helvetica", "Verdana" };

    public static final String[] buildRange = new String[] { "All",
	    "Active this month", "Active this week", "Active today" };
	...
	return Hudson.getInstance().getPluginManager()
		.getPlugin(WallDisplayPlugin.class).getVersion();
    }
	...
	if (xmlFile.exists()) {
	    ...
	    LOGGER.info(String.format("Loaded configuration data: %s",
		    config.toString()));
	} else {
	...
	Hudson.getInstance().getExtensionList(TransientViewActionFactory.class)
		.add(0, new WallDisplayTransientViewActionFactory());
	super.start();
    }
	...
	return new XmlFile(Hudson.XSTREAM, new File(Hudson.getInstance()
		.getRootDir(), WallDisplayPlugin.class.getName() + ".xml"));
    }
}
